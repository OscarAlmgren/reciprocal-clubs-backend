...
func main() {
	...
	// Initialize service
	notificationserviceService := service.NewService(repo, logger, natsClient, monitoringService)

	// Initialize handlers
	httpHandler := httpHandlers.NewHTTPHandler(notificationserviceService, logger, monitoringService)
	grpcHandler := grpcHandlers.NewGRPCHandler(notificationserviceService, logger, monitoringService)
